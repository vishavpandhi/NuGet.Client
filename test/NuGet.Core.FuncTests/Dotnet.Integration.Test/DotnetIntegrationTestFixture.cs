    ...
    public class DotnetIntegrationTestFixture : IDisposable
    {
        ...
        internal void CreateDotnetNewProject(string solutionRoot, string projectName, string args)
        {
            ...
            if (!templateDirectory.Exists)
            {
                ...
                CommandRunnerResult result = RunDotnetExpectSuccess(templateDirectory.FullName, $"new {templateArgs}");
...
{packageReferences}
    ...
    <Import Sdk='Microsoft.NET.Sdk' Project='Sdk.targets'/>
        ...
        internal CommandRunnerResult RestoreProjectExpectFailure(string workingDirectory, string projectName, string args)
            => RestoreProjectOrSolution(workingDirectory, $"{projectName}.csproj", args, expectSucccess: false);

        internal CommandRunnerResult RestoreProjectExpectSuccess(string workingDirectory, string projectName, string args)
            => RestoreProjectOrSolution(workingDirectory, $"{projectName}.csproj", args, expectSucccess: true);

        internal CommandRunnerResult RestoreSolutionExpectFailure(string workingDirectory, string solutionName, string args)
            => RestoreProjectOrSolution(workingDirectory, $"{solutionName}.sln", args, expectSucccess: false);

        internal CommandRunnerResult RestoreSolutionExpectSuccess(string workingDirectory, string solutionName, string args)
            => RestoreProjectOrSolution(workingDirectory, $"{solutionName}.sln", args, expectSucccess: true);

        private CommandRunnerResult RestoreProjectOrSolution(string workingDirectory, string fileName, string args, bool expectSucccess)
            => RunDotnet(workingDirectory, $"restore {fileName} {args}", expectSucccess);
        ...
        private CommandRunnerResult RunDotnet(string workingDirectory, string args, bool expectSuccess = true, IReadOnlyDictionary<string, string> environmentVariables = null, bool validateOutcome = true)
        {
            ...
            try
            {
                ...
                result = CommandRunner.Run(TestDotnetCli, workingDirectory, args, waitForExit: true, environmentVariables: finalEnvironmentVariables);
            ...
            void CreateDiagnostics(string args, string workingDirectory, CommandRunnerResult result, Dictionary<string, string> environmentVariables, FileInfo coreHostLogFileInfo, Exception exception = null)
            {
...
#if NET8_0
                writer.WriteLine("Assembly target framework: net8.0");
#elif NET7_0
                writer.WriteLine("Assembly target framework: net7.0");
#else
                ...
                foreach (var item in actualEnvironmentVariables.Cast<DictionaryEntry>().OrderBy(i => i.Key))
                {
        ...
        internal CommandRunnerResult PackSolutionExpectFailure(string workingDirectory, string solutionName, string args, string nuspecOutputPath = "obj", bool validateSuccess = true, string configuration = "Debug")
        ...
        private CommandRunnerResult BuildProjectOrSolution(string workingDirectory, string file, string args, bool expectSuccess, bool? appendRidToOutputPath = false)
        {
            if (appendRidToOutputPath != null)
            {
                args = $"{args} /p:AppendRuntimeIdentifierToOutputPath={appendRidToOutputPath}";
        ...
        public void Dispose()
        {
            RunDotnet(Path.GetDirectoryName(TestDotnetCli), "build-server shutdown", validateOutcome: false);
