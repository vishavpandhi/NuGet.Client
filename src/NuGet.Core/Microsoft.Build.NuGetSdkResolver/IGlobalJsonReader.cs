// Copyright (c) .NET Foundation. All rights reserved.
// Licensed under the Apache License, Version 2.0. See License.txt in the project root for license information.

using System;
using System.Collections.Generic;
using Microsoft.Build.Framework;

namespace Microsoft.Build.NuGetSdkResolver
{
    /// <summary>
    /// Represents an interface for a class that reads global.json.
    /// </summary>
    internal interface IGlobalJsonReader
    {
        /// <summary>
        /// Occurs before a file is read.
        /// </summary>
        event EventHandler<GlobalJsonFileReadEventArgs> FileReadStart;

        /// <summary>
        /// Occurs after a file is read.
        /// </summary>
        event EventHandler<GlobalJsonFileReadEventArgs> FileReadStop;

        /// <summary>
        /// Walks up the directory tree to find the first global.json and reads the msbuild-sdks section.
        /// </summary>
        /// <param name="context">An <see cref="SdkResolverContext" /> to use when locating the file.</param>
        /// /// <param name="globalJsonFullPath">Receives the full path to the global.json that was read if one was found, otherwise <c>null</c>.</param>
        /// <param name="fileName">An optional file name to search for, the default is global.json.</param>
        /// <returns>A <see cref="Dictionary{String,String}" /> of MSBuild SDK versions from a global.json if found, otherwise <c>null</c>.</returns>
        Dictionary<string, string> GetMSBuildSdkVersions(SdkResolverContext context, out string globalJsonFullPath, string fileName = GlobalJsonReader.GlobalJsonFileName);
    }
}
